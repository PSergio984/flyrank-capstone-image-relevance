...
- **Zod `strict()` object** — unknown fields rejected; safeParse on EVERY response even though Gemini JSON mode enforces the shape (research finding: JSON mode ≠ correctness). The Zod schema is the single source of truth; the request's JSON schema (`response_format.schema`, per the Interactions API naming in the research doc) is generated from it to stay in sync.
- **Flag rule**: `confidence < 0.70` marks the image `flagged_for_review`, excluded from auto-matching, visible in review API. **0.70 is a provisional default by construction** — a trust gate on the model's self-report, not a similarity threshold; the similarity thresholds have NO defaults and come only from the eval sweep (*mismatch guard rules*). Flag floor value lives in `config/thresholds.json` with provenance and is revisited when real flag-rate data exists.
