...
- **Cost attribution** (shared requirement #7): every AI call writes one `ai_cost_log` row inside the handler — `{job_id, kind: vision|embedding|post-classify, model, input_tokens, output_tokens, cost_usd}` — before the job acknowledges; a budget guard sums the ledger and refuses new batches past a configurable cap.
