...
- `corpus/manifest.json` — one entry per image: `{id, filename, category, subject, source_page_url, cdn_url, license, photographer}`. This is simultaneously the licensing evidence (Unsplash License / Pexels License both permit use; credit recorded anyway), the eval-set label anchor, and the input for the fetch script.
